 ...
 */
public class SelfCheckoutStation extends AbstractSelfCheckoutStation {
	/**
	 * Constructor utilizing the current, static configuration.
	 */
	public SelfCheckoutStation() {
		super(new ElectronicScale(), new ElectronicScale(), new TouchScreen(),
			new ReusableBagDispenser(reusableBagDispenserCapacityConfiguration), new ReceiptPrinter(),
			new CardReader(), new BarcodeScanner(), new BarcodeScanner(), new BanknoteInsertionSlot(),
		...
		for(int i = 0; i < banknoteDenominations.length; i++)
			banknoteDispensers.put(banknoteDenominations[i], new BanknoteDispenser());
